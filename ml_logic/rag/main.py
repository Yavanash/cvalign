...
# Example query (commented out properly)
...
IMPORTANT: Respond ONLY with the JSON object, no additional text or spaces.""",
    partial_variables={"format_instructions": output_parser.get_format_instructions()}
)

# Getting the 5 most relevant job descriptions to the required job
...
**Title:** {doc.metadata.get('Job Title', 'N/A')}
...
def testing():
    # Example test data (commented out)
    ...
    # 
    # test_input = CVEvalInput(target_job_desc=jd, cv=cv)
    # output = eval_chain_mistral.invoke({"target_job_desc": jd, "cv": cv})
    # 
